...
if (count($open_basedirs)){
    ...
    if (!$fm_current_root_ok) {
        ...
        foreach ($open_basedirs as $open_basedir) {
            if (strpos($fm_path,$open_basedir) !== false) {$fm_current_root = $open_basedir;
                $fm_current_root_ok = true;
                break;
            }
        }
...
function system_exec_file(){
    ...
    if ($debug_mode) return;header("Content-type: text/plain");
    $file = $fm_current_dir.$filename;
    if(file_exists($file)){
        ...
        if($entry_count){
            ...
            foreach ($entry_list as $ind=>$dir_entry) {
                ...
                if ($dir_entry['type'] == "dir") {
                    ...
                    else $dir_out[$dir_count][] = "<td class=\"sm\">&nbsp;</td>";if ( count($dir_out[$dir_count]) > $max_cells ){
    ...
    if ($reload){
        ...
        <tr><td align=right>".et('Lang').":<td>
            <select name=newlang id=newlang style=\"width:410px; padding:5px;\">
                ...
                <option value='fr'>French - by Jean Bilwes</option><option value='de'>German - by Guido Ogrzal</option>
...
function frame3(){
    ...
    if (!$about_form_was_shown) {
        echo "
        <script language=\"Javascript\" type=\"text/javascript\">about_form();
        </script>";
    }
    echo "
    </body>\n</html>";
...
function et($tag){
    ...
    $et['de']['RenderTime'] = 'Zeit, um die Seite anzuzeigen';$et['de']['Seconds'] = 's';
